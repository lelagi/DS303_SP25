...
#geom_smooth(method = "lm.quad", formula = y ~ x + x^2, color = "blue") - tried
...
```{r}
#linear regression with quad term
ggplot(Boston, aes(x = lstat, y = medv)) +
  geom_point() +
...
summary(lm.fit5)
#when thinking there is 5 movement in the data w/ 5outputs
```
...
The output means with the exponent set to 5, the model is statistically significant. It is also a good model. With each poly intercept, it .... NEED TO FINSIHED
...
```{r}
ggplot(Boston, aes(x = rm, y= age)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, color = "blue")


lm.try1 <- lm(age ~ rm, data = Boston) 
...
#second try
#lm.try <- lm(medv ~ age + I(age^2), data = Boston) 
#summary(lm.try)
...
```{r}
ggplot(Boston, aes(x = rm, y= age, I(age^2),)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x, color = "blue")

lm.try2 <- lm(age ~ rm + I(age^2), data = Boston) 

anova(lm.try1, lm.try2)
```
