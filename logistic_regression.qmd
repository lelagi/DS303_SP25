...
#scatter <- ggplot(skim_data(aes(x= numeric.mean, y= numeric.sd)) +
                    geom_point()
#scatter
```
...
```{r}
default <- glm(default ~ income, data = default, family = binomial)
summary(default)
```

Interpret the results below:

The intercept of -3.09e+00 means that this is the log odds of the default when the income is 0.

The income of -8.353e-06 means that this is the log odds of doing a default when income increases by 1.
...
```{r}
#model0 <- glm(student ~ Default, data = default, family = binomial)

#summary(model0)

model0 <- glm(default ~ student, data = default, family = binomial)
summary(model0)
```

Interpret your findings here:

The intercept of -3.50413 is statistically significant and means it is the log odds of doing a default when the student is 0 (or no).

When they are a student (studentYes), the chance of log odds increases by 0.40489 every time default goes up 1.
